...
chosenFileSubset = paste0(data_source, "/mzXML_neg/")
...
centW.integrate = 1
centW.profparam = list(step=0.01) # setting this very low, per Jan Stanstrup; low setting uses more memory but helps avoid the situation where mass accuracy eclipses the actual width of the m/z windows used to define each peak (a real possibility with Orbitrap data; see http://metabolomics-forum.com/viewtopic.php?f=8&t=598#p1853)
...
LOB <- doLOBscreen(xsA=xset_a, polarity = "negative", match.ppm = 2.5, 
                   database = default.LOBdbase$negative, exclude.oddFA = T, remove.iso = T,
                   retain.unidentified = F, rt.restrict = T)

LOBscreen_diagnostics(LOB)
LOBdata <- getLOBpeaklist(LOB) 
write.csv(LOBdata, file = "LOB_Peaklist_Neg.csv")
